/**
 * highlights-carousel.test.js — Tests Unitarios del Bloque HighlightsCarousel
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/highlights-carousel/highlights-carousel.test.js
 */
...
describe('HighlightsCarousel Block', () => {
  let block;
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./highlights-carousel.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./highlights-carousel.js');
    decorate(block);
    expect(block.classList.contains('highlights-carousel--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('highlights-carousel');
