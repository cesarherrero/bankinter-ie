/**
 * eu-institutions.test.js — Tests Unitarios del Bloque EuInstitutions
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/eu-institutions/eu-institutions.test.js
 */
...
/**
 * Crea un bloque eu-institutions mínimo para tests.
...
describe('EuInstitutions Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./eu-institutions.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./eu-institutions.js');
    decorate(block);
    expect(block.classList.contains('eu-institutions--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('eu-institutions');
