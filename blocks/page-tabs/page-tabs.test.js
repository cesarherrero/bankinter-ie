/**
 * page-tabs.test.js — Tests Unitarios del Bloque PageTabs
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/page-tabs/page-tabs.test.js
 */
...
describe('PageTabs Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./page-tabs.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./page-tabs.js');
    decorate(block);
    expect(block.classList.contains('page-tabs--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('page-tabs');
