/**
 * header.test.js — Tests Unitarios del Bloque Header
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/header/header.test.js
 */
...
/**
 * Crea un bloque header mínimo para tests.
...
describe('Header Block', () => {
  ...
  it('añade la clase de inicialización al bloque', async () => {
    ...
    decorate(block);
    expect(block.classList.contains('header--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('header');
