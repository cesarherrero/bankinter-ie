/**
 * editorial-body.test.js — Tests Unitarios del Bloque EditorialBody
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/editorial-body/editorial-body.test.js
 */
...
describe('EditorialBody Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./editorial-body.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./editorial-body.js');
    decorate(block);
    expect(block.classList.contains('editorial-body--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('editorial-body');
