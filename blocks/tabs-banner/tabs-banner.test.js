/**
 * tabs-banner.test.js — Tests Unitarios del Bloque TabsBanner
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/tabs-banner/tabs-banner.test.js
 */
...
describe('TabsBanner Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./tabs-banner.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./tabs-banner.js');
    decorate(block);
    expect(block.classList.contains('tabs-banner--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('tabs-banner');
