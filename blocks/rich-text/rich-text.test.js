/**
 * rich-text.test.js — Tests Unitarios del Bloque RichText
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/rich-text/rich-text.test.js
 */
...
describe('RichText Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./rich-text.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./rich-text.js');
    decorate(block);
    expect(block.classList.contains('rich-text--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('rich-text');
