/**
 * cookie-preference-form.test.js — Tests Unitarios del Bloque CookiePreferenceForm
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/cookie-preference-form/cookie-preference-form.test.js
 */
...
describe('CookiePreferenceForm Block', () => {
  let block;
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./cookie-preference-form.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./cookie-preference-form.js');
    decorate(block);
    expect(block.classList.contains('cookie-preference-form--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('cookie-preference-form');
