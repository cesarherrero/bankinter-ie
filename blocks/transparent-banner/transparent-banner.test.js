/**
 * transparent-banner.test.js — Tests Unitarios del Bloque TransparentBanner
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/transparent-banner/transparent-banner.test.js
 */
...
describe('TransparentBanner Block', () => {
  let block;
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./transparent-banner.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./transparent-banner.js');
    decorate(block);
    expect(block.classList.contains('transparent-banner--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('transparent-banner');
