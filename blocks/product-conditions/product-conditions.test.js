/**
 * product-conditions.test.js — Tests Unitarios del Bloque ProductConditions
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/product-conditions/product-conditions.test.js
 */
...
describe('ProductConditions Block', () => {
  let block;
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./product-conditions.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./product-conditions.js');
    decorate(block);
    expect(block.classList.contains('product-conditions--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('product-conditions');
