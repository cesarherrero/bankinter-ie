/**
 * hero-banner.test.js — Tests Unitarios del Bloque HeroBanner
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/hero-banner/hero-banner.test.js
 */
...
/**
 * Crea un bloque hero-banner mínimo para tests.
...
describe('HeroBanner Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./hero-banner.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./hero-banner.js');
    decorate(block);
    expect(block.classList.contains('hero-banner--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('hero-banner');
