/**
 * pagination.test.js — Tests Unitarios del Bloque Pagination
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/pagination/pagination.test.js
 */
...
describe('Pagination Block', () => {
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./pagination.js');
    decorate(block);
    expect(block.classList.contains('pagination--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('pagination');
