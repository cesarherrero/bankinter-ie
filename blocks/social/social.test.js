/**
 * social.test.js — Tests Unitarios del Bloque Social
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/social/social.test.js
 */
...
describe('Social Block', () => {
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./social.js');
    decorate(block);
    expect(block.classList.contains('social--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('social');
