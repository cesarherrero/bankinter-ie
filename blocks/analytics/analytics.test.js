/**
 * analytics.test.js — Tests Unitarios del Bloque Analytics
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/analytics/analytics.test.js
 */
...
describe('Analytics Block', () => {
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./analytics.js');
    decorate(block);
    expect(block.classList.contains('analytics--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('analytics');
