/**
 * highlights.test.js — Tests Unitarios del Bloque Highlights
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/highlights/highlights.test.js
 */
...
describe('Highlights Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./highlights.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./highlights.js');
    decorate(block);
    expect(block.classList.contains('highlights--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('highlights');
