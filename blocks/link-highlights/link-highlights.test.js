/**
 * link-highlights.test.js — Tests Unitarios del Bloque LinkHighlights
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/link-highlights/link-highlights.test.js
 */
...
describe('LinkHighlights Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./link-highlights.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./link-highlights.js');
    decorate(block);
    expect(block.classList.contains('link-highlights--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('link-highlights');
