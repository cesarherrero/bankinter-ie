/**
 * highlight-double.test.js — Tests Unitarios del Bloque HighlightDouble
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/highlight-double/highlight-double.test.js
 */
...
describe('HighlightDouble Block', () => {
  let block;
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./highlight-double.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./highlight-double.js');
    decorate(block);
    expect(block.classList.contains('highlight-double--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('highlight-double');
