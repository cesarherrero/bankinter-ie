/**
 * product-carousel.test.js — Tests Unitarios del Bloque ProductCarousel
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/product-carousel/product-carousel.test.js
 */
...
/**
 * Crea un bloque product-carousel mínimo para tests.
...
describe('ProductCarousel Block', () => {
  let block;
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./product-carousel.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./product-carousel.js');
    decorate(block);
    expect(block.classList.contains('product-carousel--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('product-carousel');
