/**
 * breadcrumb.test.js — Tests Unitarios del Bloque Breadcrumb
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/breadcrumb/breadcrumb.test.js
 */
...
/**
 * Crea un bloque breadcrumb mínimo para tests.
...
describe('Breadcrumb Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./breadcrumb.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./breadcrumb.js');
    decorate(block);
    expect(block.classList.contains('breadcrumb--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('breadcrumb');
