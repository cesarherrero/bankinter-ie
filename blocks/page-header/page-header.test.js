/**
 * page-header.test.js — Tests Unitarios del Bloque PageHeader
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/page-header/page-header.test.js
 */
...
/**
 * Crea un bloque page-header mínimo para tests.
...
describe('PageHeader Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./page-header.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./page-header.js');
    decorate(block);
    expect(block.classList.contains('page-header--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('page-header');
