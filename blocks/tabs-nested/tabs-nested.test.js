/**
 * tabs-nested.test.js — Tests Unitarios del Bloque TabsNested
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/tabs-nested/tabs-nested.test.js
 */
...
describe('TabsNested Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./tabs-nested.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./tabs-nested.js');
    decorate(block);
    expect(block.classList.contains('tabs-nested--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('tabs-nested');
