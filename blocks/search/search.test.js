/**
 * search.test.js — Tests Unitarios del Bloque Search
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/search/search.test.js
 */
...
/**
 * Crea un bloque search mínimo para tests.
...
describe('Search Block', () => {
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./search.js');
    decorate(block);
    expect(block.classList.contains('search--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('search');
