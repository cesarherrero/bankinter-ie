/**
 * video-banner.test.js — Tests Unitarios del Bloque VideoBanner
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/video-banner/video-banner.test.js
 */
...
/**
 * Crea un bloque video-banner mínimo para tests.
...
describe('VideoBanner Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./video-banner.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./video-banner.js');
    decorate(block);
    expect(block.classList.contains('video-banner--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('video-banner');
