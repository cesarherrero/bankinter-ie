/**
 * real-cases-grid.test.js — Tests Unitarios del Bloque RealCasesGrid
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/real-cases-grid/real-cases-grid.test.js
 */
...
describe('RealCasesGrid Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./real-cases-grid.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./real-cases-grid.js');
    decorate(block);
    expect(block.classList.contains('real-cases-grid--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('real-cases-grid');
