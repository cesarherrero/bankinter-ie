/**
 * video-carousel.test.js — Tests Unitarios del Bloque VideoCarousel
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/video-carousel/video-carousel.test.js
 */
...
/**
 * Crea un bloque video-carousel mínimo para tests.
...
describe('VideoCarousel Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./video-carousel.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./video-carousel.js');
    decorate(block);
    expect(block.classList.contains('video-carousel--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('video-carousel');
