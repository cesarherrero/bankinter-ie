/**
 * offer-maker.test.js — Tests Unitarios del Bloque OfferMaker
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/offer-maker/offer-maker.test.js
 */
...
describe('OfferMaker Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./offer-maker.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./offer-maker.js');
    decorate(block);
    expect(block.classList.contains('offer-maker--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('offer-maker');
