/**
 * faqs.test.js — Tests Unitarios del Bloque Faqs
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/faqs/faqs.test.js
 */
...
describe('Faqs Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./faqs.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./faqs.js');
    decorate(block);
    expect(block.classList.contains('faqs--initialized')).toBe(true);
  });
