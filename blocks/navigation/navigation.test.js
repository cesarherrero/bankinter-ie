/**
 * navigation.test.js — Tests Unitarios del Bloque Navigation
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/navigation/navigation.test.js
 */
...
/**
 * Crea un bloque navigation mínimo para tests.
...
describe('Navigation Block', () => {
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./navigation.js');
    decorate(block);
    expect(block.classList.contains('navigation--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('navigation');
