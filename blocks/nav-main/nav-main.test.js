/**
 * nav-main.test.js — Tests Unitarios del Bloque NavMain
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/nav-main/nav-main.test.js
 */
...
describe('NavMain Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./nav-main.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./nav-main.js');
    decorate(block);
    expect(block.classList.contains('nav-main--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('nav-main');
