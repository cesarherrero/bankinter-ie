/**
 * modal.test.js — Tests Unitarios del Bloque Modal
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/modal/modal.test.js
 */
...
/**
 * Crea un bloque modal mínimo para tests.
...
describe('Modal Block', () => {
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./modal.js');
    decorate(block);
    expect(block.classList.contains('modal--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('modal');
