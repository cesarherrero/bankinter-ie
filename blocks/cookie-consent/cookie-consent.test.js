/**
 * cookie-consent.test.js — Tests Unitarios del Bloque CookieConsent
 * Framework: vitest
 *
 * Generado por SA-D03 del Sprint 2 — Red Agéntica AEM
 * Ejecutar: npx vitest run blocks/cookie-consent/cookie-consent.test.js
 */
...
describe('CookieConsent Block', () => {
  ...
  it('importa y ejecuta decorate() sin errores', async () => {
    const mod = await import('./cookie-consent.js');
  ...
  it('añade la clase de inicialización al bloque', async () => {
    const { default: decorate } = await import('./cookie-consent.js');
    decorate(block);
    expect(block.classList.contains('cookie-consent--initialized')).toBe(true);
  });
  ...
  it('no lanza errores con un bloque vacío', async () => {
    ...
    emptyBlock.classList.add('cookie-consent');
